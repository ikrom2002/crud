...
def dislike_article(requst,slug):
    article=Article.objects.get(slug=slug)         

    if requst.user not in article.dislikes.all():
        article.dislikes.add(requst.user)
        article.likes.remove(requst.user)
    elif requst.user in article.dislikes.all():
        article.dislikes.remove(requst.user)
    return redirect('article_detail', slug=slug)        
